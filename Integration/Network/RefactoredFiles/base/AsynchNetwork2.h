...
void Network::m_messageHandler(char message[]){
  ...
  while(token != NULL){
    ...
    if(strcmp(token, "*CTSGPS") == 0 && CTSGPSFLAG == false){
      Serial.println("Recieved CTS GPS ");
      CTSGPSFLAG = true;
      globalFlag = true;
    }else if(strcmp(token, "*RTSGPS") == 0 && RTSGPSFLAG == false){
      Serial.println("Recieved RTS GPS ");
      RTSGPSFLAG = true;
      globalFlag = true;
    }else if(strcmp(token, "*CTSPACKET") == 0 && CTSGPSFLAG == true && CTSPACKETFLAG == false){
      Serial.println("Recieved CTS PACKET ");
      CTSPACKETFLAG = true;
      globalFlag = true;
    }else if(strcmp(token, "*RTSPACKET") == 0 && RTSGPSFLAG == true && RTSPACKETFLAG == false){
      Serial.println("Recieved RTS PACKET ");
      RTSPACKETFLAG = true;
      globalFlag = true;
    }else if(strcmp(token, "*PACKET") == 0 && RTSGPSFLAG == true && RTSPACKETFLAG == true && PACKETFLAG == false){
      PACKETFLAG = true;
      globalFlag = true;
