...
private:
  void m_trackRTSMessages();
  void m_trackCTSMessages();
  void m_queueID();
  void m_enqueueID();
  bool m_canServiceNewRover();
  int m_numRovers;
  vector<int> m_RoverIDs;
};
...
void Base::m_trackRTSMessages(){

}
void Base::m_trackCTSMessages(){

}
void Base::m_queueID(int ID){
  m_RoverIDs.push(ID);
}
void Base::m_enqueueID(int ID){
  m_RoverIDs.pop_back();
}
